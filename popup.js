document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateSidebarAndDropdown() {
        // 提取所有唯一标签
        const tags = [...new Set(allPrompts.map(p => p.tag).filter(t => t && t.trim() !== ''))];

        // A. 更新弹窗内的自定义下拉框内容
        if (tags.length === 0) {
            tagDropdown.innerHTML = '<div style="padding: 10px; color: #94a3b8; font-size: 12px; text-align: center;">暂无已有标签</div>';
        } else {
        ...
        tagDropdown.querySelectorAll('.tag-option').forEach(option => {
            option.onmousedown = (e) => { // 使用 onmousedown 确保在 onblur 之前触发
        ...
        // B. 更新左侧侧边栏菜单
        const currentFilter = document.querySelector('.nav-item.active')?.dataset.filter || 'all';
        const existingLinks = navItemsContainer.querySelectorAll('a.nav-item');
        existingLinks.forEach(link => link.remove());

        // 插入“全部”
        const allLink = createNavLink('all', '全部提示词', allPrompts.length, currentFilter === 'all');
    ...
    function renderCards() {
        ...
        displayList.forEach((item, index) => {
            const matchesSearch = item.name.toLowerCase().includes(term) || item.content.toLowerCase().includes(term);
            ...
            if (matchesSearch && matchesCategory) {
                ...
                card.onclick = (e) => {
                    if (['pin-btn', 'edit-btn', 'delete-btn'].some(cls => e.target.classList.contains(cls))) return;
    ...
    saveBtn.onclick = () => {
        ...
        if (editingIndex !== null) {
            allPrompts[editingIndex] = { ...allPrompts[editingIndex], name, tag, content };
        } else {
    ...
    tagInput.onblur = () => {
        // 延迟隐藏，给点击选项留出时间
        setTimeout(() => { tagDropdown.style.display = 'none'; }, 200);
    };

    // --- 6. 基础 UI 事件 ---
    addBtn.onclick = () => { resetForm(); modal.style.display = 'flex'; };
